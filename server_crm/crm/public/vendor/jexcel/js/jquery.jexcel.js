/**
 * (c) 2013 Jexcel Plugin v1.4.0 | Bossanova UI
...
var methods = {
    ...
    init : function( options ) {
        ...
        prepareTable = function () {
            ...
            // Number of columns
            var size = options.colHeaders.length;
    ...
    createTable : function() {
        ...
        for (i = 0; i < options.colHeaders.length; i++) {
            ...
            var header = options.colHeaders[i];
            var headerClass = options.colHeaderClasses[i] || '';

            // Column type hidden
            if (options.columns[i].type == 'hidden') {
                // TODO: when it is first check the whole selection not include
                tr += '<td id="col-' + i + '" style="display:none;" title="' + header + '" class=" ' + headerClass + '">' + header + '</td>';
            } else {
                // Other column types
                tr += '<td id="col-' + i + '" width="' + width + '" align="' + align +'" title="' + header + '" class=" ' + headerClass + '">' + header + '</td>';
            }
        }
                            ...
                            if (typeof(options.contextMenu) == 'function') {
                                ...
                                if ($(e.target).parent().parent().is('thead')) {
                                    ...
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].allowDeleteColumn == true) {
                                        contextMenuContent += "<a onclick=\"jQuery('#" + $.fn.jexcel.current + "').jexcel('deleteColumn'," + o[1] + ")\">Delete this column<span></span></a><hr>";
                                    }
                                    ...
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].editable == true) {
                                        ...
                                        if ($.fn.jexcel.defaults[$.fn.jexcel.current].columns[columnId[0]].type != 'readonly') {
                                            ...
                                            if (! $($.fn.jexcel.selectedCell).hasClass('edition')) {
                                                ...
                                                if ((e.keyCode == 110 || e.keyCode == 190 || e.keyCode == 32) || (e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode >= 65 && e.keyCode <= 90) || (e.keyCode >= 96 && e.keyCode <= 105)) {
    ...
    setValue : function(cell, value, force) {
        ...
        if ($(cell).length) {
            ...
            // Update cells
            $(this).jexcel('updateCells', records, null, force);
     ...
     * @return void
     */
    updateCells : function(cells, ignoreHistory, force)
    {
        // Id
        var id = $(this).prop('id');
    ...
    getRowData : function(row) {
       // Get row
       var row = $(this).find('#row-' + row).parent().find('td').not(':first');
    ...
    getColumnData : function(columnNumber) {
        ...
        // Selected row
        var cells = $(this).find('tbody%20.html' + columnNumber);
    ...
    undo : function () {
        ...
        if ($.fn.jexcel.defaults[id].historyIndex >= 0) {
            ...
            // Update cells
            $(this).jexcel('updateCells', records, true, true);
    ...
    redo : function () {
        ...
        if ($.fn.jexcel.defaults[id].historyIndex < $.fn.jexcel.defaults[id].history.length - 1) {
            ...
            // Update cells
            $(this).jexcel('updateCells', historyRecord.cellChanges, true, true);
